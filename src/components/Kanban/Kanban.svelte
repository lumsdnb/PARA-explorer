<script lang="ts">
  ...
  function handleDrop(event: DragEvent) {
    ...
    const toCardColumnIndex = parseInt(
      target.parentElement!.dataset.columnIndex!
    );
    const fromCardIndex = columns[fromCardColumnIndex].cards.findIndex(
      (card) => card.id === cardId
    );
    const toCardIndex = Array.from(
      target.parentElement!.querySelectorAll('.card')
    ).indexOf(target);

    if (fromCardIndex > -1) {
      const card = columns[fromCardColumnIndex].cards.splice(
        fromCardIndex,
        1
      )[0];
